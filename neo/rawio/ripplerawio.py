...
class RippleRawIO(BaseRawIO):
    ...
    def __init__(self, filename=None, nfx_override=None, nev_override=None, nfx_to_load=None, verbose=False):
        """
        ...
        self._filenames = {}
        if nfx_override:
            self._filenames["nfx"] = re.sub(os.path.extsep + "nf[1,2,3,4,5,6]$", "", nfx_override)
        else:
        ...
        for ext in self.extensions:
            if ext.startswith("nf"):
                file2check = "".join([self._filenames["nfx"], os.path.extsep, ext])
            else:
    ...
    def _parse_header(self):
        ...
        for nfx_nb in self._avail_nfx:
            ...
            # read nfx headers
            self.__nfx_basic_header[nfx_nb], self.__nfx_ext_header[nfx_nb] = self.__nfx_header_reader[spec](nfx_nb)
        ...
        assert all(
            nfx_nb in self._avail_nfx for nfx_nb in self.nfx_to_load
        ...
        if len(self.nfx_to_load) > 0 and self.__nfx_spec[self.nfx_to_load[0]] == "2.1" and not self._avail_files["nev"]:
            pass
        ...
        if len(self.nfx_to_load) > 0:
            ...
            assert all(
                nb == nb_segments_for_nfx[0] for nb in nb_segments_for_nfx
            ...
            for data_bl in range(self._nb_segment):
                ...
                for nfx_nb in self.nfx_to_load:
                    ...
                    if self.__nfx_data_header[nfx_nb] is None:
                        ...
                        t_start = (
                            self.__nfx_data_header[nfx_nb][data_bl]["timestamp"]
    ...
    def __extract_nfx_file_spec(self, nfx_nb):
        ...
        if nfx_file_id["file_id"].decode() == "NEURALSG":
            ...
            spec = "{}.{}".format(nfx_file_id["ver_major"], nfx_file_id["ver_minor"])
        else:
    ...
    def __read_nfx_header_variant_a(self, nfx_nb):
        ...
        nfx_ext_header = np.memmap(filename, shape=shape, offset=offset_dt0, dtype=dt1, mode="r")
    ...
    def __read_nfx_dataheader_variant_a(self, nfx_nb, filesize=None, offset=None):
        """
    ...
    def __read_nfx_dataheader_variant_b(
        ...
        while offset < filesize:
            ...
            data_size = int(dh["nb_data_points"]) * int(self.__nfx_basic_header[nfx_nb]["channel_count"]) * 4
    ...
    def __read_nfx_data_variant_a(self, nfx_nb):
        ...
        # get shape of data
        shape = (self.__nfx_params["2.1"](nfx_nb)["nb_data_points"], self.__nfx_basic_header[nfx_nb]["channel_count"])
        offset = self.__nfx_params["2.1"](nfx_nb)["bytes_in_headers"]

        # read nfx data
        # store as dict for compatibility with higher file specs
        data = {0: np.memmap(filename, dtype="float32", shape=shape, offset=offset, mode="r")}
    ...
    def __read_nfx_data_variant_b(self, nfx_nb):
        ...
        for data_bl in self.__nfx_data_header[nfx_nb].keys():
            ...
            offset = self.__nfx_data_header[nfx_nb][data_bl]["offset_to_data_block"]

            # read data
            data[data_bl] = np.memmap(filename, dtype="float32", shape=shape, offset=offset, mode="r")
    ...
    def __read_nev_data(self, nev_data_masks, nev_data_types):
        ...
        dt0 = [("timestamp", "uint32"), ("packet_id", "uint16"), ("value", "S{}".format(data_size - 6))]
    ...
    def __match_nfx_and_nev_segment_ids(self, nfx_nb):
        ...
        if self.__nev_spec == "2.3":
            ...
            for k, (data, ev_ids) in self.nev_data.items():
                ...
                for i, seg in enumerate(list_nonempty_nfx_segments[:]):
                    ...
                    end_of_current_nfx_seg = (
                        seg["timestamp"] + seg["nb_data_points"] * self.__nfx_basic_header[nfx_nb]["period"]
                    )

                    mask_after_seg = (ev_ids == i) & (data["timestamp"] > end_of_current_nfx_seg + nfx_period)

                    # Show warning if spikes do not fit any segment
                    #  (+- 1 sampling 'tick')
                    # Spike should belong to segment before
                    mask_outside = (ev_ids == i) & (data["timestamp"] < int(seg["timestamp"]) - nfx_offset - nfx_period)
                    ...
                    if len(data[mask_after_seg]) > 0:
                        ...
                        if i == len(list_nonempty_nfx_segments) - 1:
                            timestamp_resolution = self.__nfx_params[self.__nfx_spec[nfx_nb]](
                                "timestamp_resolution", nfx_nb
                            )
                            time_after_seg = (
                                data[mask_after_seg]["timestamp"][-1] - end_of_current_nfx_seg
            ...
            assert nb_possible_nev_segments == len(
                nonempty_nfx_segments
            ), "Inconsistent nf{0} and nev file. {1} segments present in .nev" "file, but {2} in nf{0} file.".format(
                nfx_nb, nb_possible_nev_segments, len(nonempty_nfx_segments)
            )

            new_nev_segment_id_mapping = dict(zip(range(nb_possible_nev_segments), sorted(list(nonempty_nfx_segments))))
    ...
    def __get_nfx_param_variant_a(self, nfx_nb):
        ...
        bytes_in_headers = (
            self.__nfx_basic_header[nfx_nb].dtype.itemsize
            + self.__nfx_ext_header[nfx_nb].dtype.itemsize * self.__nfx_basic_header[nfx_nb]["channel_count"]
        )
        ...
        nfx_parameters = {
            "nb_data_points": int(
                ...
                / (2 * self.__nfx_basic_header[nfx_nb]["channel_count"])
                - 1
            ),
            "labels": labels,
            ...
            "min_digital_val": np.array([-1000] * self.__nfx_basic_header[nfx_nb]["channel_count"]),
            ...
            "time_unit": pq.CompoundUnit("1.0/{}*s".format(30000 / self.__nfx_basic_header[nfx_nb]["period"])),
        }
    ...
    def __get_nfx_param_variant_b(self, param_name, nfx_nb):
        ...
        """
        nfx_parameters = {
            "labels": self.__nfx_ext_header[nfx_nb]["electrode_label"],
            "units": self.__nfx_ext_header[nfx_nb]["units"],
            "min_analog_val": self.__nfx_ext_header[nfx_nb]["min_analog_val"],
            "max_analog_val": self.__nfx_ext_header[nfx_nb]["max_analog_val"],
            ####################
            ...
            ####################
            "timestamp_resolution": self.__nfx_basic_header[nfx_nb]["timestamp_resolution"],
            "bytes_in_headers": self.__nfx_basic_header[nfx_nb]["bytes_in_headers"],
            "sampling_rate": self.__nfx_basic_header[nfx_nb]["timestamp_resolution"]
            / self.__nfx_basic_header[nfx_nb]["period"]
            * pq.Hz,
            "time_unit": pq.CompoundUnit(
                "1.0/{}*s".format(
                    self.__nfx_basic_header[nfx_nb]["timestamp_resolution"] / self.__nfx_basic_header[nfx_nb]["period"]
                )
            ),
        }
